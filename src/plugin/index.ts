...
const LOSSLESS_RECALL_POLICY_PROMPT = [
  ...
  "- Use `lcm_grep(..., allConversations: true)` for broad discovery across sessions before narrowing.",
  "- Use `lcm_expand_query(..., allConversations: true)` when the answer likely spans multiple sessions; this is bounded synthesis, not exhaustive expansion.",
  "- Use explicit `conversationId` when the question is clearly about one known session.",
